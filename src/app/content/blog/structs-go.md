...
A **gestão de memória** em **Go** não é manual como em linguagens como **C** ou **C++** (`malloc`/`free`); ele usa um algoritmo que analisa estaticamente o fluxo de como vai ser alocado na **memória**. Isso ocorre em tempo de compilação: se a variável é mantida em **Stack** ou "escapa" para a **Heap**.

### Mas como assim "escapa"?

O **Escape Analysis** é uma fase do compilador que determina se uma variável pode ser mantida na **Stack** ou se ela deve "escapar" para a **Heap**.
...
Para isso ficar ainda melhor, vamos ver como o pacote interno [`cmd/compile/internal/escape`](https://github.com/golang/go/tree/master/src/cmd/compile/internal/escape) funciona por baixo dos panos.
...
### Como evitar o escape para a Heap

**Value Semantics**: quando retornamos um valor, ele é copiado para a **stack frame** da função chamadora.

- **Foge para a Heap**: `func New() *User` (o objeto precisa sobreviver à função).
- **Fica na Stack**: `func New() User` (o objeto é copiado).

> Essa é a maneira mais simples de evitar o escape. Vou deixar só essa maneira aqui, mas tem outras formas de evitar. O importante é entender o conceito. Pode ser um conteúdo para outro post, até porque o foco aqui é sobre **structs**.

## Memória Física

Beleza, o **Escape Analysis** decidiu que uma variável deve ser alocada na **Heap**. Mas, até aí, o compilador só deixou a "tag"; é aí que entra o **runtime**. O **Go** não se utiliza de um sistema de gerenciamento de memória tradicional como o `malloc` do **C**; ele implementa um alocador baseado em **TCMalloc** (**Thread-Caching Malloc**).

> O **TCMalloc** é um alocador de memória desenvolvido pelo **Google**, projetado para alta concorrência. Ele funciona criando pequenos caches locais para cada thread (no **Go**, para cada processador lógico **P**). Isso permite que a maioria das alocações ocorra sem a necessidade de travas (**locks**), eliminando o gargalo que surge quando múltiplas partes de um programa tentam pedir memória ao mesmo tempo.

![O TCMalloc](/images/posts/structs/tcmalloc_internals.png)

### 1. mcache: O Cache Local

Cada processador lógico possui um `mcache`. Quando uma **struct** escapa para a **Heap**, o **Go** tenta alocar aqui.

- Isso gera um custo de **trava zero**: como o cache é exclusivo daquele processador, não há concorrência com outras threads. O que é tão rápido quanto alocar na **Stack**.
...
Para que isso seja eficiente, o **Go** utiliza as **Size Classes**:
...
Aqui é onde o design da sua **struct** começa a cobrar o preço. O **Go** sempre aloca na "caixa" que melhor cabe o dado, arredondando para cima. Se sua **struct** não preenche o slot completamente, o espaço restante é desperdiçado.
...
O **Go** fornece o pacote `unsafe` para você medir exatamente o tamanho das suas estruturas no terminal.
...
A regra de ouro de organizar campos do maior para o menor é poderosa, mas não deve ser uma lei absoluta.
...
- **fieldalignment**: esta ferramenta (parte do pacote `golang.org/x/tools`) analisa seu código e sugere a ordem exata dos campos para reduzir o **padding**.
- **Linters**: muitos linters de **CI/CD** (como o `golangci-lint`) já possuem regras para detectar **structs** ineficientes.

Tentei introduzir alguns assuntos de memória e até do **scheduler** do **Go**, mas o foco aqui era sobre as **structs**, então deixei só o essencial para entender como a memória funciona por baixo dos panos. Se quiser, posso fazer um post só sobre o **scheduler** e o **runtime** do **Go**; tem muita coisa interessante lá.